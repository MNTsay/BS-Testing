...
*/
double print_result_short(const PractRand::TestResult &result) {
	...
	if (true) {// 10 characters?
		...
		if (raw > 99999.0) std::printf("+inf");
		else if (raw < -99999.0) std::printf("-inf");
		else if (std::fabs(raw) < 999.95) std::printf("%+.1f", raw);
		else std::printf("%+.0f", raw);
	...
	if (true) {// 14 characters?
		if (result.type == result.TYPE_PASSFAIL)
			...
			std::printf((result.type != result.TYPE_GOOD_P) ? "~ " : "= ");
			if (a > 0.49) {
				...
				if (dig > 999) std::printf("%d  ", (s > 0) ? 1 : 0);
					...
					if (s > 0) std::printf("1-%.0fe-%.0f  ", sig, dig);
					else       std::printf("%.0fe-%.0f  ", sig, dig);
//					if (dig < 100) std::printf(" ");
//					if (dig < 10) std::printf(" ");
				}
			}
			else if (a > 0.4) std::printf("%.2f  ", p);
			else              std::printf("%.1f  ", p);
		}
		else if (result.type == result.TYPE_BAD_S || result.type == result.TYPE_GOOD_S) {
			double s = result.get_suspicion();
			double p = result.get_pvalue();
			std::printf((result.type == result.TYPE_BAD_S || result.type == result.TYPE_RAW_NORMAL) ? "~ " : "= ");
			if (p >= 0.01 && p <= 0.99) std::printf("%.3f  ", p);
				...
				if (dig > 9999) std::printf("%d  ", (s > 0) ? 1 : 0);
					...
					if (s > 0) std::printf("1-%.1fe-%.0f  ", sig, dig);
					else       std::printf("%.1fe-%.0f  ", sig, dig);
//					if (dig < 100) std::printf(" ");
//					if (dig < 10) std::printf(" ");
				}
			}
	...
	if (true) {// 17 characters?
		...
		*/
		...
		if (false) ;
		else if (rs >999) std::printf("16");
		else if (rs >325) std::printf("15");
		else if (rs >165) std::printf("14");
		else if (rs > 85) std::printf("13");
		else if (rs > 45) std::printf("12");
		else if (rs > 25) std::printf("11");
		else if (rs > 17) std::printf("10");
		else if (rs > 12) std::printf("9");
		else if (rs >8.5) std::printf("8");
		else if (rs >6.0) std::printf("7");
		else if (rs >4.0) std::printf("6");
		else if (rs >3.0) std::printf("5");
		else if (rs >2.0) std::printf("4");
		else if (rs >1.0) std::printf("3");
		else if (rs >0.0) std::printf("2");
		else              std::printf("1");
		std::printf("  ");
	}
	std::printf("\n");
...
double print_result(const PractRand::TestResult &result, bool print_header = false) {
	if (print_header) std::printf("  Test Name                         Raw       Suspicion     Processed     Evaluation\n");
	//                                     10        20        30      1  40        50        60        70        80
	std::printf("  ");//2 characters
	//NAME
	if (true) {// 34 characters
		std::printf("%-*s", 34, result.name.c_str());
	}
...
//	std::printf("rng=%s", tman->get_rng()->get_name().c_str());
	std::printf("%s  ", tman->get_rng()->get_name().c_str());
...
//	std::printf(" %s), time= ", mode_unit_names[mode]);
	std::printf(" %s)  ", mode_unit_names[mode]);
...
//		double decimal_suspicion = print_result(results[marked[i]], i == 0);
		double decimal_suspicion = print_result_short(results[marked[i]]/*, i == 0*/);
